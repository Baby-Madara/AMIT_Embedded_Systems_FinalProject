...
void Read_all_Struct_EEPROM(Users_userNode *pointer_struct)
{
	...
	for (; Counter_One < Number_of_User;)
	{ // loop 1 as the action will be done /user  although nested loop is bad it cant be avoided here
...
{
	// check for value of 0xAA if value == 146 every thing is fine
	if (EEPROM_read(0xFF) == 146)
	{
		// return 0 if no proplem
		return 0;
	}
